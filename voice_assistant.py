...
today = ("Hello Today's date is "+ str(date.today()))
...
print("Welcome User! Mark here.")
print(today)
s.speak("Hello Sir I am mark! Your Personal Assistant.What do you want me to address you as")
user=input("Your Name:")
s.speak("hello {}".format(user))
...
while True:
	with sr.Microphone() as source:
		...
		try:
			...
			if(('introduce' in x) or ('what are you' in x) or ('who are you' in x) or (('something' in x) and ('yourself' in x)) or ('about youself' in x)):
...
while True:
	...
	with sr.Microphone() as source2:
		...
		req=r.listen(source2)


		try:
			sound=r.recognize_google(req)
			print(sound)
			...
			print("Please try saying that again")
